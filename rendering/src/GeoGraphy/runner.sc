...
/*	pause { arg aID ; actantDict[aID].pause ; this.changed(this, 
		[\handling, \pause, actantDict[aID].aID]) } */ // pause is not implemented in Routine. It was in Task (OLD implementation)
	...
	*/
	...
	update { arg theChanged, theChanger, more; 
		...
		case 
			...
			more = [\actant].addAll(more.add(offsetWeight).add(statsDict[more[0]])) ;			// format of more arr:
			//[\actant, vID, vertex, eID, edge, aID, weight, offsetWeight, count]
			this.changed(this, more) ;
				}
			{ (theChanged.class == Actant).and(more[2].isNil) } //this means that the graph sequence is finish. No loop
