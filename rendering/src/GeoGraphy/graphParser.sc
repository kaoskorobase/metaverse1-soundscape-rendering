...
//assign positon with an algorithm
...
Layout {	
	...
	*new { arg v, algo ;  
	...
	uniform { arg minX = 0, maxX = 1200, minY = 0, maxY = 790 ;	//modify for soundscape case, before --> maxY=800
	...
	*new { arg graph, runner, painter ;  
	...
	initGraph { arg aGraph, aRunner, aPainter ;
		...
		specials = ["e+", "e-", "a+", "a-", "t+", "v+", "v-", "i-", "c+", "r+",
			"s+", "o+", "d+", "d-", "w+", "p+", "z+", "x+"] ;	//z+ Add for soundscape
...
//
		...
		//specify Vertex offsetVertexListenedArea = vopts	//add by Mattia for SoundScape: CartoonModel
		z+ [name val]

		//specify Vertex offsetVertexListenedArea = vopts by index
		x+ [id val]
		*/
	...
	parseLine { arg aString ;
		...
		{ arr[0] =="z+" }			//add for Soundscape implementation by M
			{ //"change offsetVertexListenedArea standard".postln ; 
			graph.setoffsetVertexListenedArea( this.getKey(arr[1]), arr[2].asFloat) ;
			this.toLog(arr) }

		{ arr[0] =="x+" }			//add for Soundscape implementation by M
			{ //"change offsetVertexListenedArea standard".postln ; 
			graph.setoffsetVertexListenedArea( arr[1].asFloat, arr[2].asFloat) ;
			this.toLog(arr) }
...
ParserGUI {
	...
	*new { arg graphParser ; // runner is needed for tempo
	...
	/*	doc = Document.listener.title_("Nim Chimpsky's Post Window")
			...
			.stringColor_(Color(1, 1, 1))		//Modify for Soundscape development
			.bounds_(Rect(50, 50, 300, 800)) ;	//because windows doesn't implement it */
 ...
 ;
		...
		w.front ;
		w.onClose_({ /* doc			
//Modify for Soundscape development
//because windows doesn't implement it
