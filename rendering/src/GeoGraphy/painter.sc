...
Painter {
	...
	var <>graph, <>runner, <>w, <>vStaticDict, <>labelList, <>geoListener, <>buttListener;	//geoListener add by M for soundscape cartoon model visualisation 
	var <>wiring ;			
	...
	*new { arg graph, runner, geoListener, vertices = true, edges = true, vLabels = true, eLabels = true, //geoListener add by M
	...
	/////////////////add by M for GeoListener init visualisation
		if (geoListener != nil,
			{
			a = geoListener.la;
			b = geoListener.lb;
	...
	drawEdges { 					
		...
		w.drawHook = {
			...
			GUI.pen.width = 0.5;	
			GUI.pen.color = Color.new(1, 1, 1, 0.5) ; // Bug: the edges are no visible after setbackground
			// plot  edges
...
///// Required if you want to force update, e.g. outside a routine
	...
	drawListener { arg a,b,orient;	//add by M  //orient for future implementation
		var butt,vLab;
		vLab = "LISTENER";
		if (vPosScale == nil, {	
			},{
		...
		butt = GUI.staticText
			.new(w, Rect.new(a-(lviewdim), b-(lviewdim*0.25), lviewdim*2, lviewdim*0.5)) //a circle will be better, not possibility with Sc gui tools
			.background_(Color(1, 1, 1, 0.7)) 					//will be better to create the interface with processing or similar
			.string_(vLab)
			.align_(\center)
	...
	update { arg theChanged, theChanger, more;
		...
		{ chger == \position }	//add for soundscape implementation by M	
			{
			buttListener.remove;
			a = more[1];
			b = more[2];
