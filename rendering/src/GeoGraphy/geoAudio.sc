...
// this in GraphParser. Labels must match filenames
//e+ basso1 1 basso2 1 basso3 1 basso4 1 basso5 1 basso1 a+ basso1

// you pass a folder, and a list of filenames (i.e. vertex labels)
// extension is by default waw, you can omitt it

//h = Sampler.new(b).initAudio("/audioRumentario/clean/basso/", [\basso1, \basso2, \basso3, \basso4, \basso5], "wav") ;

// class
// the same of GraphSampler but you pass a rootpath and a list of names
...
Sampler : GeoAudio  {
	...
	initAudio { arg aSamplesPath, nameList, ext = "wav" ;		
			...
			server.doWhenBooted({				
						nameList.do({ arg name ;
		 					bufDict.add(name
			 						-> Buffer.read(server, samplesPath++name++"."++ext))

								}) ;
						}) ;
			}
	
			sendDef {		//the synthDef being sent		
					var aDef = SynthDef(\Sampler, { arg bufnum, amp = 1, out = 0, dur = 1 ; //prima era graphSampler modificato da Mattia
	...
	play { arg message ;		//[\actant, vID, vertex, eID, edge, aID, weight, offsetWeight, count]
			...
			amp = (weight+offsetWeight).thresh(0) ;	
	 		if (bufDict[label] != nil,
		  			{ 	dur = bufDict[label].numFrames/server.sampleRate ;
						Synth.new(\Sampler,		//prima era graphSampler modificato da Mattia
		 				[\bufnum, bufDict[label].bufnum, \amp, amp, \dur, dur]) }	//\Pan , pan	come modifico il pan? se mando un mess con il nuovo valore di pan ogni volta che c'è un update da geoListener e devo fare in modo di non riattivare il campione
											//ipoteticamente basterebbe un controllo, se il vertice del messaggio è il vertice che sta ancora suonando (vdur) allora modifica solo il panning, amp, e filter e non riattivare
...
GraphSampler : GeoAudio  {		//la riscriviamo in una nuova classe samples
	...
	initAudio { arg aSamplesPath ;
		...
		bufDict = IdentityDictionary.new ;	
		server.doWhenBooted({
			p = Pipe.new("dir" + samplesPath, "r") ;				l = p.getLine ;
			while({l.notNil}, {
				case 
				{ l.contains("\\") }
